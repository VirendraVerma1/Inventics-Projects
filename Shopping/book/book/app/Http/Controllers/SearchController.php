...
class SearchController extends Controller
{
    ...
    public function search_by_category(Request $request,$name)
    {
        ...
        $this->updatedata();
        $wished_items=$this->showAllWishlist();
        $tempinventory=$this->searchinventory($name,"Price");
        ...
        return view('SearchPage.index',compact('tempcategory','inventory','allBrands','name','totalProductCount','viewLimit','wished_items'));
    }
    ...
    public function get_filtered_product(Request $request)
    {
        ...
        foreach($new_inventory as $inv)
        {
            ...
            {
                if($inv->brand==$request->brandName && $inv->min_price>=$request->priceVal && $inv->min_price<=$request->PRICEMAXVal )
                {
            ...
            {
                if($inv->min_price>=$request->priceVal && $inv->min_price<=$request->PRICEMAXVal)
                {
                    array_push($completeinventory,$inv);
                }
            }
        ...
        $return_HTML=view('SearchPage.product',compact('inventory','wished_items','totalProductCount'))->render();
        return json_encode(array('data'=>$return_HTML,'viewLimit'=>$viewLimit));;
       // return view('layout.Search_Page.Filters.maincontent',compact('inventory'));
    }
